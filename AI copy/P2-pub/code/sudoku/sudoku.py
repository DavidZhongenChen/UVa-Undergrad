...
class Sudoku:
    ...
    def firstEpsilonVariable(self):
        ...
        NOTE: for the sake of the autograder, please do the search by highest row, then the leftmost column:
        for r in row:
            for c in col:
        """
        for r in range(9):
    ...
    def updateFactor(self, factor_type, i):
        ...
        # values = self.factorRemaining[factor_type, i]
        # nums = []
        # if factor_type == BOX:
        #     nums = self.box(self, i)
        # if factor_type == ROW:
        #     nums = self.row(self, i)
        # if factor_type == COL:
        #     nums = self.col(self, i)
        # violations = crossOff(values,nums)
        # self.factorNumConflicts[factor_type, i] = violations
    ...
    def updateVariableFactors(self, variable):
        ...
        """
        # raise NotImplementedError()
        self.updateFactor(BOX, int(variable[0]/3) * 3 + int(variable[1])/3)
    ...
    def getSuccessors(self):
        ...
        assignments = []
        domain = self.variableDomain(nextVariable[0], nextVariable[1])
        for i in domain:
            assignments.append(self.setVariable(nextVariable[0], nextVariable[1], i))
        return assignments
    ...
    def randomRestart(self):
        ...
        for r in range(9):
            ...
            for c in range(9):
                if self.board[r][c] == 0:
                    self.board[r][c] = remaining[remaining_index]
                    remaining_index += 1
    ...
    def randomSwap(self):
        ...
        while(True):
            ...
            if ((row, col1) in self.fixedVariables or (row, col2) in self.fixedVariables):
                continue
    ...
    def gradientDescent(self, variable1, variable2):
        ...
        """
        # raise NotImplementedError()
        current_num_conflicts = self.numConflicts()
        self.modifySwap(variable1,variable2)
        swapped_num_conflicts = self.numConflicts()
        if current_num_conflicts < swapped_num_conflicts:
            if(random.randint(1, 1000) != 1):
                self.modifySwap(variable1,variable2)
        # print current_num_conflicts
