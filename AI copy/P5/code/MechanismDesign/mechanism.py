...
class PostedPrice:
    ...
    def setObjectiveOptimizeRevenue(self):
        ...
        """
        optimal_expected_revenue = 0
        optimal_price = 0
        for val in self.values:
            current_revenue = 0 #keep track of revenue for when value = val
            for i in range(len(self.dist)):
                if self.values[i] >= val:
                    current_revenue += self.dist[i] * val
            if optimal_expected_revenue < current_revenue:
                optimal_expected_revenue = current_revenue
                optimal_price = val
        return optimal_expected_revenue, optimal_price
...
class Mechanism:
    ...
    def addICConstraint(self):
        ...
        value[i] * p[i] - x[i] >= value[i] * p[j] - x[j]
        """
        for i in range(10):
            for j in range(10):
    ...
    def addFConstraint(self):
        for i in range(10):
            self.C_F.append(self.p[i] >= 0)
            self.C_F.append(self.p[i] <= 1)
    ...
    def setObjective(self):
        ...
        lp1 = op(sum( - self.x[i] * self.dist[i] for i in range(10)), self.C_IC + self.C_IR + self.C_F) #concatanate the constraints
        lp1.solve()
        return -float(str(lp1.objective.value()[0])), self.x, self.p
